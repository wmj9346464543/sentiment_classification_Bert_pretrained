...
def collate_fn(batch):
    ...
    batch_token_ids = [bertTokenizer.encode(text, max_length=max_len, truncation=True) for text in texts]#对超出长度的句子进行截断
    ...
    batch_token_ids = [token_ids + [bertTokenizer.pad_token_id] * (batch_max_len - len(token_ids)) if len(#填充标记
